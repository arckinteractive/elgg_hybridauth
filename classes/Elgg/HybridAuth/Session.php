...
class Session {
	 ...
	 * @return bool
	 */
	public function addAuthRecord(Provider $provider, $uid) {
		elgg_trigger_plugin_hook('hybridauth:authenticate', $provider->getName(), array('entity' => $this->user));
	...
	public function removeAuthRecord(Provider $provider) {
		elgg_trigger_plugin_hook('hybridauth:deauthenticate', $provider->getName(), array('entity' => $this->user));
	...
	public function authenticate(Provider $provider, $addauth = true) {
		try {
			...
			$this->save();
			if ($profile && $addauth) {
				$this->addAuthRecord($provider, $profile->identifier);
			}
		} catch (\Exception $e) {
